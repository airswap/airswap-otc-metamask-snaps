...
import { BIP_32_PATH_REGEX, BIP_39_PATH_REGEX, MIN_BIP_44_DEPTH, SLIP_10_PATH_REGEX, CIP_3_PATH_REGEX } from "./constants.mjs";
import { getCurveByName } from "./curves/index.mjs";
import { derivers } from "./derivers/index.mjs";
 ...
 */
export function validatePathSegment(path, hasKey, depth) {
    if (path.length === 0) {
    ...
    if (depth === MIN_BIP_44_DEPTH && (!startsWithBip39 || path.length !== 1)) {
        throw new Error(`Invalid HD path segment: The segment must consist of a single BIP-39 node for depths of ${MIN_BIP_44_DEPTH}. Received: "${String(path)}".`);
    }
    ...
    if (pathWithoutKey.length > 0) {
        ...
        assert(pathWithoutKey.every((segment) => segment.startsWith(`${firstSegmentType}:`)), `Invalid HD path segment: Cannot mix 'bip32' and 'slip10' path segments.`);
    }
}
/**
